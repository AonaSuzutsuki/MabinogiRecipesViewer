...
namespace CookInformationViewer.Models.Db.Loader
{
	...
	public class TemplateLoader
    {
        private class ValueIndentPair
        {
            ...
            public bool IsIndent { get; init; }
        }

        #region Fields
        private readonly string _templateText;

        private Dictionary<string, ValueIndentPair> _map = new Dictionary<string, ValueIndentPair>();
        #endregion

        /// <summary>
        /// Initialize TemplateLoader with path of template file.
        /// </summary>
        /// <param name="templateText">Path of template file.</param>
        public TemplateLoader(string templateText)
        {
        ...
        public void Assign<T>(string key, T value, bool isIndent = false)
        {
            if (_map.ContainsKey(key))
                _map[key] = new ValueIndentPair() { Value = value.ToString(), IsIndent = isIndent };
            else
                _map.Add(key, new ValueIndentPair() { Value = value.ToString(), IsIndent = isIndent });
        }
        ...
        public void Reset()
        {
            _map = new Dictionary<string, ValueIndentPair>();
        }
        ...
        private string Analyze()
        {
            ...
            var lines = _templateText.UnifiedBreakLine().Split('\n');
